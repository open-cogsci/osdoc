...
接下来，添加相关频道（`cogsci`）和（`conda-forge`）并安装所有相关包。确保 `pyqode.core` 和 `pyqode.python` 来自 `cogsci` 频道, 且版本 >= 3.2，而不是来自 `conda-forge` 频道的旧版本。
...
OpenSesame 是一个常规的 Python 环境，您可以按照描述使用 `pip` 或 `conda` 升级软件包：
