...
Nous allons créer une expérience catégorique amorçage. Les participants voient brièvement une chaîne de lettres et doivent décider si la chaîne de lettres est un mot ou un non-mot (i.e., une tâche de décision lexicale). Pour indiquer sa décision, le participant doit appuyer sur une touche du clavier. Notre hypothèse est que la reconnaissance d'un mot sera plus vite quand la catégorie du mot est déjà amorcée. Pour amorcer la catégorie, on montrera un mot indiquant la catégorie (par ex. "ANIMAL") juste avant la présentation de la cible (par ex. 'lapin'). On s'appelle ce mot amorçant un *Prime*. Comme condition de contrôle, on présentera également des mots sans avoir amorcé leur catégorie. En tel cas, la cible est précédée par une chaîne de lettres non-informative (par ex. "XXXXX").
...
Finalement, on enregistre notre nouvelle expérience en cliquant l'icône 'Enregistrer' de la barre d'outils principale.
...
En total, ça donne 12 Cible * Prime combinaisons différentes. Ces 12 conditions constituent la liste d'essais d'un bloc (voyez %hierarchy). Cette liste doit être remplit dans l'élément *block_loop*.  
...
Il est souvent pratique de dire OpenSesame quelle est la bonne réponse par essai en définissant la variable 'correct_response'. Ça permet OpenSesame de suivre les variables de la performance, comme 'acc' (pourcentage correct).  
...
Comme illustré à la %trialSeq1, l'objectif est de construire une séquence d'essai comme suit:
...
Cependant, il faut qu'on fasse un petit changement sur cet élément quand même. Actuellement, la durée de ce `sketchpad` est mis sur 'keypress' (appui). Ça veut dire que le point de fixation sera présenté jusqu'a le sujet appuye sur un touche. Ce n'est pas ce qu'on veut. On veut que le point de fixation sera présenté par 500 ms (voyez %trialSeq1), et que, aprés, l'expérience avance automatiquement.  
...
- Cliquez sur l'icone "Ab" pour sélectionner l'outil texte
- Cliquez sur le centre de l'écran
- Cela ouvrira une boîte de dialogue vous demandant de spécifier un texte
- Au lieu de taper le Prime (par ex. 'ANIMAL') directement, on tape *le nom de la variable indépendante* ('prime') entre crochets
- Enfin, mettez la durée du *prime* sur 100 ms

Les crochets indique que nous avons affaire à une texte variable. Donc, au lieu de présenter '[prime]' sur l'écran, OpenSesame présentera la valeur de la variable (i.e. 'XXXXXX' ou 'ANIMAL') comme tirée de la liste *block_loop*.
...
Ensuite, on met la durée du `sketchpad` *cible* sur 0 ms. Ça peut vous semblez contre-intuitif, mais ça veut juste dire que OpenSesame va initialiser la prochaine élément (ici, *keyboard_response*) tout de suite. L'élément *keyboard_item* lui même ne change pas ce qui est actuellement montrer au sujet sur l'écran.
...
On voit que la séquence de la session experimentale entiere se compose de 3 éléments:
...
- Supprimez le texte qui est actuellement montré:
	- Sélectionnez la flêchede la barre d'outils
	- Cliquer sur le texte au centre du `sketchpad`
	- Appuyez la touche 'delete' du clavier
- Ajouter un nouveau text:
	- Cliquer sur l'icone 'Ab' pour sélectionner l'outil texte
	- Cliquer quelque part sur le `sketchpad`
	- Ecrit une petite texte qui informe le sujet de sa tâche de décision lexical
...
- Mettez ce message en français en utilisant l'outilt texte.
...
- Ouvrez le sketchpad 'feedback_incorrect' et répétez cette procédure. Assurez-vous que, pour l'*évaluation_incorrect*, le point de fixation le couleur soit rouge ('red').
...
- Cliquez sur l'élément *trial_sequence* dans la vue d'ensemble'. Un onglet s'ouvre, qui donne une vue d'ensemble de chaque événement de la séquence. A droite, on voit les déclarations 'Run-if'. Ils indiquent dans quelles circonstances OpenSesame exécutera chaque élément. Maintenant, ils sont tous exécutés 'always' ('toujours'). Ce valeur est correct pour tous nos éléments, sauf les éléments *juste* et *faux*'. Effectivement, à la fin de chaque essai, il faut seulement exécuter un des deux, en dépendant si le participant a répondu juste ou faux. 
...
Imaginez que nous ne sommes pas seulement intéressés sur l'effet du Prime, mais qu'on veut examiner si l'effet du Prime interagit avec l'effet de la durée de l'interval entre Prime et Cible. Par exemple, on peut élargir la question de recherche vers:
	
"La reconnaissance d'un mot, est-elle plus vite quand la catégorie du mot est amorcée très récemment, mais non quand l'interval entre Prime et Cible augmente?"
...
figure:
 ...
 source: hypoth2.png
 caption: |
  Notre hypothèse est qu'il y aura un effet d'interaction entre Prime et Interval.
--%

Pour répondre à cette question, il faut un deuxième variable indépendente: durée de l'interval. On va le donner deux valeurs différentes: 1000 ms vs. 3000 ms.
...
- On peut ajouter une variable dans notre liste de l'*experimental_loop*. Par conséquence, l'interval sera varié seulement entre blocs (on aura un bloc entier avec un interval de 1000 ms, et un autre avec un interval de 4000 ms.
...
- Changez le sketchpad *interval* d'un sketchpad *invariable* vers un sketchpad *variable*. 
- Utilisez la méthode 'entre crochets' dans la boîte 'Duration'

%--
figure:
 id: variable3
 source: variable3.png
 caption: |
